# E12-T2: Publish VRX-64-slide to crates.io
...
Prepare `VRX-64-slide` for publication on crates.io: fill out all required metadata, set MSRV, ensure the crate compiles to both `wasm32-wasip1` (slides) and the host target (engine), write enough documentation to be useful on docs.rs, and set up a GitHub Actions workflow that publishes on tag. Perform the initial `cargo publish`.

## Background

`VRX-64-slide` is the single most important crate in the VZGLYD ecosystem. Every slide in the world depends on it. Publishing it to crates.io lets slide authors work in completely isolated repos without path dependencies back to the engine monorepo.
...
`VRX-64-slide/Cargo.toml` is minimal:
...
documentation = "https://docs.rs/VRX-64-slide"
readme = "README.md"
...
### Step 2 — Create VRX-64-slide/README.md

A short README visible on crates.io:

- One paragraph: what is `VRX-64-slide`, what is VZGLYD
- The Cargo.toml dependency snippet (`VRX-64-slide = "0"`)
...
`VRX-64-slide` must compile on both targets that use it:

```bash
# Engine (host)
cargo check -p VRX-64-slide

# Slides (WASM)
cargo check -p VRX-64-slide --target wasm32-wasip1
```
...
jobs:
  publish:
    ...
    steps:
      ...
      - run: cargo publish -p VRX-64-slide
        env:
          CARGO_REGISTRY_TOKEN: ${{ secrets.CARGO_REGISTRY_TOKEN }}
```

Tag convention: `VRX-64-slide-v0.1.0`. This allows `vzglyd_sidecar`, the engine, and slide repos to all release independently on their own tags.
...
VRX-64-slide = { path = "../VRX-64-slide" }  # keep path for workspace builds
...
- [ ] `VRX-64-slide/Cargo.toml` has description, license, repository, keywords, categories, documentation, readme, rust-version
- [ ] `VRX-64-slide/README.md` exists with dependency snippet, minimal example, ABI policy link
- [ ] `cargo check -p VRX-64-slide` and `cargo check -p VRX-64-slide --target wasm32-wasip1` both pass
...
- [ ] The crate is live on crates.io at `https://crates.io/crates/VRX-64-slide`
...
| File | Change |
|------|---------|
| `VRX-64-slide/Cargo.toml` | Add all publication metadata |
| `VRX-64-slide/README.md` | New — crates.io landing page |
| `VRX-64-slide/src/lib.rs` | Add top-level module doc with example |

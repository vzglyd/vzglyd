...
3. **Core crates are unpublished.** `VRX-64-slide` (the ABI contract) and `vzglyd_sidecar` (the networking library) are path-only dependencies. A slide author working in their own repository cannot reference them from crates.io — they must vendor or fork. Publishing these is the prerequisite for the entire third-party slide ecosystem.

4. **No ABI stability commitment.** Nothing documents what `VRX-64-slide` version guarantees mean, which changes are breaking, or what compatibility window the engine maintains. Without this policy, third-party slides break unpredictably on engine updates.
...
- A slide author with no access to the engine repo can create, build, and publish a slide using only `VRX-64-slide` and `vzglyd_sidecar` from crates.io.
- The engine repo contains only the engine and the loading slide. All other slides are in their own repos.
- `VRX-64-slide` and `vzglyd_sidecar` are on crates.io with a documented semver policy.
...
| E12-T2 | Publish VRX-64-slide to crates.io | P0 | M | E12-T1 | E12-T3, E12-T4 |
...
| E12-T11 | VRX-64-get slide installer CLI | P3 | M | E12-T10 | - |
| E12-T12 | README, docs, and OSS hygiene files | P0 | L | - | - |
| E12-T13 | Slide authoring guide | P1 | M | E12-T2 | - |
| E12-T14 | VRX-64-slide and vzglyd_sidecar rustdoc | P1 | M | E12-T2 | - |
...
- E12-T2 Publish VRX-64-slide (needs T1)
